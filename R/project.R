...
proj4.str2list <- function(pp){
  ...
  strstart <- gregexpr("\\+[[:alpha:]]", pp)[[1]]
  strend <- c(strstart[2:length(strstart)] - 1, nchar(pp))
  p1 <- mapply(function(x, y) substring(pp, x + 1, y), strstart, strend)  # vector of "x=n" strings
